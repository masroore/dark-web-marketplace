...
class SearchController extends Controller
{
    ...
    private function order($collection, $orderQuery)
    {
        ...
        if ($orderQuery !== null) {
            if ($orderQuery == 'price_asc') {
                $ordered = $collection->sortBy(fn ($product) => (float) $product->price_from);
            }
            if ($orderQuery == 'price_desc') {

                $ordered = $collection->sortByDesc(fn ($product) => $product->price_from);
            }
            if ($orderQuery == 'newest') {
                $ordered = $collection->sortByDesc(fn ($product) => $product->created_at);
            }
        }
    ...
    private function priceFilter($collection, $minPriceQuery, $maxPriceQuery)
    {
        ...
        if ($minPriceQuery !== null && (float) $minPriceQuery > 0) {
            $minPrice = (float) $minPriceQuery;
            $filteredCollection = $collection->filter(fn ($product) => $product->price_from >= (float) $minPrice);
        }
        // max price
        if ($maxPriceQuery !== null && (float) $maxPriceQuery > 0) {
            $maxPrice = (float) $maxPriceQuery;
            $filteredCollection = $collection->filter(fn ($product) => $product->price_from <= (float) $maxPrice);
        }
