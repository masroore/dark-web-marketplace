...
class catalog_model
{
        ...
        if (!empty($_POST)) {
            if (isset($_POST['payment_methods'])) {
                $this->User->updatePrefs(
                    [
                        'CatalogFilter' => [
                            'cryptocurrencies' => $_POST['payment_methods'] == $_POST['payment_method_options']
                                    ? false
    ...
    public function fetchContinentsCountries()
    {
        ...
        if (false != $stmt_Continent && false != $stmt_Country) {
            ...
            while ($stmt_Country->fetch()) {
                $continents[$country_continent_id]['Countries'][] = $countries[$country_id] = [
                    'ID' => $country_id,
                    'Name' => $country_name,
                    'Top' => $country_top,
                ];
    ...
    public function fetchListingCategories(
        ...
        array_walk(
            ...
            function ($array) use (&$allCategories): void {
        ...
        if (
            ...
            $activeCategory = $defaultListingCategoryID ?: $allowedListingCategoryID;
        } elseif (
            $activeCategory
    ...
    public function fetchListings(
        ...
        if (
            ...
            array_walk(
                $listings,
                function (&$listing) use ($cryptocurrency, $sortPerUnit, &$listingCryptocurrencies): void {
                    if ($listing['GroupID']) {
    ...
    public function fetchFAQCategories()
    {
        ...
        if (false !== $stmt_getFAQCategories && false !== $stmt_getFAQTitles) {
            ...
            return [$faq_categories, $stmt_getFAQTitles->num_rows];
    ...
    public function fetchFAQs($category_id, $query = false)
    {
        ...
        $stmt_fetchFAQCategory_types_params = $stmt_fetchFAQs_params = [];
        ...
        if (false !== $stmt_fetchFAQs && false !== $stmt_fetchFAQCategory) {
            ...
            $stmt_fetchFAQCategory_args = array_merge(
                [$stmt_fetchFAQCategory_types],
                $stmt_fetchFAQCategory_params
            );
            call_user_func_array(
                [$stmt_fetchFAQCategory, 'bind_param'],
            ...
            if ($stmt_fetchFAQCategory->num_rows == 1) {
                ...
                $stmt_fetchFAQs_args = array_merge(
                    [$stmt_fetchFAQs_types],
                    $stmt_fetchFAQs_params
                );
                call_user_func_array(
                    [$stmt_fetchFAQs, 'bind_param'],
                    $stmt_fetchFAQs_args
                );
                ...
                while ($stmt_fetchFAQs->fetch()) {
                    $faqs[] = [
                        ...
                        'title' => $faq_title,
                        'content' => preg_replace_callback('~\[constant="(.*?)"\]~', fn ($matches) => constant($matches[1] != strip_tags($matches[1]) ? $matches[1] : '<p>' . $matches[1] . '</p>'), $faq_content),
        ...
        if (false !== $stmt_fetchFAQs) {
            ...
            while ($stmt_fetchFAQs->fetch()) {
                $faqs[] = [
                    ...
                    'category_id' => $faq_category_id,
                    'title' => $faq_title,
                    'content' => preg_replace_callback('~\[constant="(.*?)"\]~', fn ($matches) => constant($matches[1]), $faq_content),
                    'vote' => ($faq_vote == 1 || $faq_vote_count == 0),
                ];
            }
