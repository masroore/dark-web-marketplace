...
class account_model
{
    ...
    public function renderUserQueryGraph($queryIdentifier): void
    {
    ...
    private function checkVendorApplicationStatus($policy = false): void
    {
    ...
    public function getDashboardNotifications(): void
    {
        /*list(
    ...
    public function generateUserBackup(): void
    {
        ...
        [$pgp, $encrypt_pgp] = $this->User->Info('PGP', 'EncryptPGP');
        if ($encrypt_pgp == 1) {
    ...
    public function fetchContinentsCountries()
    {
        ...
        if (false != $stmt_Continent && false != $stmt_Country) {
            ...
            while ($stmt_Country->fetch()) {
                $continents[$country_continent_id]['countries'][] = [
                    'id' => $country_id,
                    'name' => $country_name];
            }
    ...
    public function updateSettings()
    {
        if (!empty($_POST)) {
            ...
            if (!empty($update)) {
                ...
                $stmt_args = array_merge([$stmt_types], $stmt_params);
				...
				')) {
                    ...
                    call_user_func_array([$stmt_updateUser, 'bind_param'], $stmt_args);
    ...
    public function sendMessage()
    {
        ...
        [$my_pgp_encrypt, $my_pgp] = $this->User->Info(0, 'EncryptPGP', 'PGP');
		...
		')) {
            ...
            if ($stmt_getListing->num_rows == 1) {
                ...
                $content = [
                    ...
                    'price' => $currency_id != CURRENCY_ID_BTC
    ...
    public function fetchListings($sort, $page)
    {
            ...
            if ($listing_count > 0) {
                ...
                while ($stmt_getListings->fetch()) {
                    $listings[] = [
                        ...
                        'groupHue' => $listingGroupNumber !== null
        ...
        return [$listing_count, $listing_inactive_count, $listings];
    }
    ...
    public function new_listing()
    {
        if (!empty($_POST)) {
            ...
            if (isset($_POST['ships_to_continent'])) {
                ...
                $shipping_name = $shipping_description = $shipping_price = $shipping_currency = $shipping_pricing_mode = $shipping_options = [];
			...
			')) {
                ...
                $stmt_args = array_merge([$stmt_types], $stmt_params);
                call_user_func_array([$stmt_insertListing, 'bind_param'], $stmt_args);
                ...
                if ($stmt_insertListing->execute()) {
                    ...
                    $paymentMethods = array_filter(
                        ...
                        fn ($paymentMethodID) => $this->checkUserPaymentMethod($paymentMethodID)
                    )
                        ?: [];
                    ...
                    if (!empty($listing_attributes) && count($listing_attributes) > 0) {
						...
						')) {
                            ...
                            $stmt_attributes_args = array_merge([$stmt_attributes_types], $stmt_attributes_params);

                            call_user_func_array([$stmt_insertAttributes, 'bind_param'], $stmt_attributes_args);
                    ...
                    if (!empty($_POST['ships_to_continent'])) {
                        ...
                        if (count($stmt_continents_params) / 2 > 0) {
							...
							')) {
                                ...
                                $stmt_continents_args = array_merge([$stmt_continents_types], $stmt_continents_params);

                                call_user_func_array([$stmt_insertContinents, 'bind_param'], $stmt_continents_args);
                            ...
                            if (!empty($_POST['ships_to_country'])) {
                                ...
                                $stmt_countries_params = [];

                                foreach ($_POST['ships_to_country'] as $key => $ship_to_country) {

                                    if (!is_numeric($ship_to_country)) {
                                        continue;
                                    }

                                    if (!in_array($country_continents[$ship_to_country], $_POST['ships_to_continent'])) {
                                        continue;
                                    }
                                ...
                                if (count($stmt_countries_params) / 2 > 0) {
									...
									')) {
                                        ...
                                        $stmt_countries_args = array_merge([$stmt_countries_types], $stmt_countries_params);

                                        call_user_func_array([$stmt_insertCountries, 'bind_param'], $stmt_countries_args);
    ...
    public function edit_listing($listing_id)
    {
        if (!empty($_POST)) {
			...
			')) {
                ...
                $stmt_args = array_merge([$stmt_types], $stmt_params);
                call_user_func_array([$stmt_updateListing, 'bind_param'], $stmt_args);
        ...
        if (!empty($_POST)) {
            ...
            if (false !== $stmt_updateDescription && false !== $stmt_updateSection && false !== $stmt_insertSection) {
                ...
                if ($section_ids) {
                    ...
                    call_user_func_array(
                        [$stmt_deleteSections, 'bind_param'],
    ...
    private function makeRecursive($d, $r = 0, $pk = 'Parent', $k = 'ID', $c = 'Children')
    {
        $m = [];
        foreach ($d as $e) {
            isset($m[$e[$pk]]) ? false : $m[$e[$pk]] = [];
            isset($m[$e[$k]]) ? false : $m[$e[$k]] = [];
    ...
    public function uploadFile(
        ...
        if (
            ...
            if (
                $check_image
                && @!([$img_width, $img_height] = getimagesize($_FILES[$file_field]['tmp_name']))
            ) {
    ...
    private function fixVendorPublicKey(): void
    {
    ...
    public function generatePGPMessage($pgp = false, $session_variable_name = 'PGP_authentication_code', $compare_pgp = false)
    {
        ...
        if ($compare_pgp) {
            $_SESSION[$session_variable_name]['pgp'] = $public_key_ascii;
        }
    ...
    private function checkPGP($response, $session_variable_name = 'PGP_authentication_code', $pgp = false)
    {
        ...
        if ((isset($session['verified']) && $session['verified'] == $pgp) || ($response == $session['answer'] && (!$pgp || $pgp == $session['pgp']))) {
            if ($pgp) {
                $_SESSION[$session_variable_name]['verified'] = $pgp;
            }
    ...
    public function generateMessage($type, $public_key): void
    {

        $authentication_code = hash('sha256', uniqid(openssl_random_pseudo_bytes(16), true));

        $derivation_i = !empty($_SESSION['temp_derivation_path']) ? $_SESSION['temp_derivation_path'] : mt_rand(11, 892);
        ...
        $_SESSION['BTC_authentication_code'] = [
