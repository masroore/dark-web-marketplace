...
class forum_model
{
        ...
        if (
            ...
            foreach ($forumEntries_result as $forumEntry) {
                ...
                $forumEntries[] = [
                    ...
                    'posterImage' => NXS::getPictureVariant(
                    ...
                    'highlighted' => $forumEntry['NewEntries']
                        || $forumEntry['Announce']
                    ...
                    'posterFlair' => !$forumEntry['PosterIsAnonymous']
                            ...
                            : false,
                    'recentCommenterFlair' => !$forumEntry['MostRecentCommenterAnonymous']
                            ? $this->fetchUserFlairs(
    ...
    public function fetchDiscussions($category, $page, $sort, $query = false)
    {
        ...
        if (false !== $stmt_countDiscussions && false !== $stmt_getDiscussions) {
            if (!empty($stmt_vars)) {
                $stmt_args_count = array_merge([$stmt_types], $stmt_vars);
                call_user_func_array([$stmt_countDiscussions, 'bind_param'], $stmt_args_count);
            }
            ...
            if ($discussion_count > 0) {
                ...
                $stmt_args = array_merge([$stmt_types], $stmt_vars);
                call_user_func_array([$stmt_getDiscussions, 'bind_param'], $stmt_args);
            ...
            return [$discussion_count, $discussions];
        }
        exit(); // $this->db->error);
    ...
    public function fetchComments($discussion_id, $sort, $page)
    {
        ...
        if (false !== $stmt_getDiscussion) {
            ...
            $stmt_getComments_where = ['`Discussion_Comment`.`DiscussionID` = ?'];
            ...
            if ($stmt_getDiscussion->num_rows == 1) {
                ...
                if ($stmt_getComments->num_rows > 0) {
                    ...
                    while ($stmt_getComments->fetch()) {
                        ...
                        $comments[] = [
                            'ID' => $comment_id,
                            'posterAlias' => $anonymousCommenterID !== null
                                    ? '<em>Anonymous</em>' . (
                            ...
                            'posterImage' => $anonymousCommenterID === null
                                    ? NXS::getPictureVariant($comment_poster_image, IMAGE_THUMBNAIL_SUFFIX)
                                    : false,
                            'posterSignature' => $anonymousCommenterID === null
                                    ? $comment_poster_signature
                                    : false,
                            ...
                            'rawContent' => $this->User->IsMod
                                    ? $comment_content_raw
                                    : preg_replace('/(\[quote[^\]]*) date=[^\]]*(\])/is', '$1$2', $comment_content_raw),
                            'posterFlairs' => $anonymousCommenterID === null
                                    ? $this->fetchUserFlairs(
                                        $comment_poster_id,
                                        true,
                                        false,
                            ...
                            'anonymousHueRotateDeg' => $anonymousCommenterID !== null
                                    ? (
                    ...
                    if ($listingName) {
                        ...
                        $listing = [
                            ...
                            'price' => NXS::formatPrice($currency, $listingEURPrice) .
                                (
                                    $listingPerUnit
                                        ? ' / ' . $listingPerUnit
                                        : false
                                ),
                            'price_crypto' => $listingMinimumQuantity
                ...
                if ($comment_id || $recent_comment_count == 0) {
                    ...
                    if (preg_match_all("/\\[quote=['\"]?(\\d+)['\"]?\\]((?:(?!\\[\\/quote).)*)\\[\\/quote\\]/is", $content, $comment_quotes) > 0) {
                        ...
                        $content = preg_replace_callback(
                            ...
                            fn ($matches) => '[quote=' . $matches[1] . ' date=' . $quote_dates[$matches[1]] . ']' . $matches[2] . '[/quote]',
                            $content
                        );
                    }
