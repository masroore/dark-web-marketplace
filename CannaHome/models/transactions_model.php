...
class transactions_model
{
    ...
    private function _sortUnspentOutputs(&$inputs): void
    {
    ...
    public function fetchTransactions(
        ...
        return [$transaction_count, $transactions];
    ...
    private function processExpiredTransactions(): void
    {
    ...
    private function startTransactionDispute($transactionID): void
    {
        if (
            $this->db->qQuery(
                "
    ...
    public function _playground(): void
    {
        set_time_limit(0);
        ...
        if (false !== $stmt_getTransaction) {
            ...
            if ($stmt_getTransaction->num_rows == 1) {
                ...
                if (!empty($listing_quantity)) {
                    $text_quantity = $listing_quantity * $order['Quantity'] . ' ' . ($listing_quantity * $order['Quantity'] == 1 ? $unit_singular : ($unit_abbreviation ?: $unit_plural));
                } else {
                    $text_quantity = false;
                }
        ...
        if ($promoCode = $promoCode ?: $_POST['promo_code']) {
            ...
            if ($promoDiscount) {
                ...
                if (isset($order['Price']['price_shipping'])) {
                    $finalPrice += $order['Price']['price_shipping'];
                }

                $isFree = false;
                if ($promoDiscount['isPercentage']) {
                    $finalPrice *= (1 - $promoDiscount['Discount'] / 100);
    ...
    public function prepareTransaction($transaction)
    {
        ...
        if ($hadTooLowToProcess) {
            $_SESSION['temp_notifications']['tooLowToProcess'] = [
            ...
            $_SESSION['temp_notifications']['transactionPreparationError'] = [
            ...
            default:
                ...
                if (
                    ...
                    && $transactionIDs = array_map(
                        fn ($transactionIdentifier) => $this->getTransactionID($transactionIdentifier),
    ...
    public function getWithdrawableTransactions($option)
    {
        ...
        $stmt_transactions_params = [$this->User->ID];
    ...
    public function _setTransactionsPlaced($transactionIDs): void
    {
    ...
    public function checkUnconfirmedRejectedTransactions(): void
    {
    ...
    public function signTransactions($cryptocurrencyID)
    {
        ...
        $withdrawnTransactionIDs = $usedAddresses = [];
        $errors = false;
    ...
    public function getDisputeMessages($transactionID, $page)
    {
        ...
        if (false !== $stmt_countDisputeMessages && false !== $stmt_getDisputeMessages) {
            ...
            if ($dispute_message_count > 0) {
                ...
                return [$page, $dispute_message_count, $dispute_messages];
            }
        ...
        if (
            ...
            if ($stmt_checkTransaction->num_rows == 1) {
                ...
                if (!empty($_POST['message'])) {
                    ...
                    $message_array = $message_array_vendor = $message_array_buyer = $message_array_mediator = [
                        'Type' => 'message',
                        'Time' => date('j M Y H:i'),
            ...
            if ($stmt_getProposal->num_rows == 1) {
                ...
                if ($message['Type'] == 'proposal') {
                    ...
                    try {
                        ...
                        return [false, $transactionID, false];
                    }
                        ...
                        if (
                            ...
                            if ($message['Proposal']['Content']['isVerdict']) {
                                if (
                                    ...
                                    return [true, $transactionID, 'feedback'];
                                }
                            } elseif (
                                ...
                                */
                                ...
                                if ($stmt_updateProposal->execute()) {
                                    return [true, $transactionID, 'dispute'];
                                }
                            }
                        }
                    ...
                    switch ($proposal['Type']) {
                        case 'refund':
                            if (
                                ...
                                return ['tx/' . $transactionIdentifier . '/', $transactionID];
                            }
                            ...
                            break;
                        case 'reship':
                            $new_tx = array_merge($transactions, [
    ...
    private function getUnspentOutputs(
        ...
        if ($requiredConfirmations >= TX_CONFIRMATIONS_ELECTRUM_CONFIRMED) {
            ...
            if (
                ...
                return [$inputs_value, $inputs];
            }
        }

        if ($showError) {
            $_SESSION['temp_notifications']['indeterminate_outputs'] = [
