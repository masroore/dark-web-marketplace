 ...
 *
 * @see https://github.com/dincek/PHPNodeJS/
 *
...
class NodeJS
{
     ...
     *
     * @param bool $debug Enable or disable debug messages
     */
    public function __construct($debug = false, $timeZone = 'Etc/UTC')
    {
        ...
        if ($this->debug) {
            ...
            self::DebugMsg('Current Execution Path: ' . __DIR__ . '/');
            self::DebugMsg();
        }
    }
     ...
     * @param bool   $jQuery			 Load jQuery library [defailt: false]
     *
     * @return string   Result of executing JavaScript
     */
    public function run($javascript_code, $function_name = '', $args = [], $jQuery = false)
    {
        $tmpFile = tempnam(__DIR__, 'PHPNodeJSWrapper');
        ...
        $command = 'cd ' . __DIR__ . '/ && ' . $this->NodePath . ' ' . $this->PHPNodeJSWrapper . ' ';
        ...
        if (mb_strlen(serialize((array) $args), '8bit') > 30000) {
            $tmpArgsFile = $tmpFile = tempnam(__DIR__, 'PHPNodeJSWrapper_args');
     ...
     * @param bool $jQuery Load jQuery library [defailt: false]
     */
    private function SetJSWrapper($jQuery = false): void
    {
        if ($this->debug) {
		...
		<?php
        $data = strtr(ob_get_clean(), ['<script type="text/javascript">' => '', '</script>' => '']);
     ...
     * Cleanup JS Wrapper.
     */
    private function CleanJSWrapper(): void
    {
        if ($this->debug) {
     ...
     */
    private static function DebugMsg($msg = ''): void
    {
        if ($msg == '') {
