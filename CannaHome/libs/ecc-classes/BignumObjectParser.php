...
abstract class BignumObjectParser implements Parsable
{
    ...
    public function getIdentifier()
    {
        ...
        if (Identifier::isLongForm($firstOctet)) {
            throw new LogicException(sprintf('Identifier of %s uses the long form and must therefor override "Object::getIdentifier()".', static::class));
        }
    ...
    protected function setContentLength($newContentLength): void
    {
    ...
    protected static function parseIdentifier($identifierOctet, $expectedIdentifier, $offsetForExceptionHandling): void
    {
    ...
    protected static function parseContentLength(&$binaryData, &$offsetIndex, $minimumLength = 0)
    {
        ...
        if ($contentLength < $minimumLength) {
            throw new ParserException('A ' . static::class . " should have a content length of at least {$minimumLength}. Extracted length was {$contentLength}", $offsetIndex);
        }
