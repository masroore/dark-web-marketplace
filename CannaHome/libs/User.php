...
class User
{
    ...
    public function __construct(Database $db, $member = false)
    {
        $this->db = $db;
    ...
    public function getListingPaymentMethods(
        ...
        if (
            ...
            $activePaymentMethods = array_filter(
                $allPaymentMethods,
                fn ($paymentMethod) => $paymentMethod['Available']
            );
            $activePaymentMethodIDs = array_map(
                fn ($paymentMethod) => $paymentMethod['ID'],
    ...
    public function Info()
    {
        ...
        if (!$ignoreCache) {
            $cachedResults = $m->getMulti(
                array_map(
                    fn ($stat) => MEMCACHED_KEY_PREFIX_USER_INFO . $user_id . $stat,
                    $args
                )
            );
        }
        ...
        if (!empty($attributes)) {
            ...
            if ($stmt_UpdateStats = $this->db->prepare($query)) {
                call_user_func_array([$stmt_UpdateStats, 'bind_param'], $bind_vars);
                ...
                if ($stmt_UpdateStats->num_rows == 1) {
                    ...
                    call_user_func_array([$stmt_UpdateStats, 'bind_result'], $variables);
    ...
    {
        $new_attributes = ['Preferences' => array_merge($this->Attributes['Preferences'], $new_prefs)];
    ...
    private function getDefaultNotifications(): void
    {
		...
		")) {
            ...
            exit(); // STATEMENT PREPARATION FAILED
        }

        // New User Alias Notification
        if (isset($_SESSION['new_user']) && $_SESSION['new_user']) {
            $this->Notifications->custom('<strong>Welcome to ' . $this->db->getSiteInfo('SiteName_Short') . '!</strong> You have been assigned a random alias. <strong>Click here</strong> to choose your own', URL . 'account/settings/#profile', '?do[DismissNewUserNotification]', 'General', [
                'Color' => 'blue',
