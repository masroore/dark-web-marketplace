...
class Thread extends Eloquent
{
    ...
    {
        $users = $this->participants()->select('user_id')->get()->map(fn ($participant) => $participant->user_id);
    ...
    public function scopeForUserWithNewMessages($query, $userId)
    {
        ...
        return $query->join($participantTable, $this->getQualifiedKeyName(), '=', $participantTable . '.thread_id')
            ...
            ->where(function ($query) use ($participantTable, $threadsTable): void {
    ...
    {
        return $query->whereHas('participants', function ($q) use ($participants): void {
    ...
    public function addParticipant($userId): void
    {
        ...
        collect($userIds)->each(function ($userId): void {
    ...
    {
        $query = DB::select(
            'select s.title, s.id as sid, u.role, u.username, u.admin_role_type from participants p
...
where thread_id = :tid and user_id != :uid and p.deleted_at is NULL',
    ...
    public function userUnreadMessages($userId)
    {
        ...
        return $messages->filter(fn ($message) => $message->updated_at->gt($participant->last_read));
    }
