...
class UpdateBitcoinRates extends Command
{
    ...
    public function handle($attempt = 0)
    {
        ...
        try {
            if (!property_exists($response, 'USD')) {
                throw new AssertionError('Property USD not exists.');
            }
            if (!property_exists($response, 'RUB')) {
                throw new AssertionError('Property RUB not exists.');
            }
