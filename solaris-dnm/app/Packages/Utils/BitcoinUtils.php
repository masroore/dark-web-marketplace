...
class BitcoinUtils
{
    ...
    {
        try {
            if (self::getRate(self::CURRENCY_RUB) === 0) {
                throw new AssertionError('RUB rate is 0.');
            }
            if (self::getRate(self::CURRENCY_USD) === 0) {
                throw new AssertionError('USD rate is 0.');
            }
        ...
        try {
            ...
            if ($response->getStatusCode() !== 200) {
                throw new BitcoinException(
                    'Bitcoind ' . $command->getMethod() . ' failed: ' .
