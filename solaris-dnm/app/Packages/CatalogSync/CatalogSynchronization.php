...
class CatalogSynchronization
{
    ...
    private $bitcoinBlockCount;

    private $bitcoinConnections;
    ...
    {
        try {
            if (null === $this->bitcoinConnections) {
            ...
            if (null === $this->bitcoinBlockCount) {
    ...
    protected function getGoodsState()
    {
        ...
        foreach ($goods as $good) {
            ...
            $item = [
                ...
                'has_quests' => $good->whereHas('packages', function ($packages): void {
                    $packages->where('has_quests', true);
                })->exists(),
                'has_ready_quests' => $good->whereHas('packages', function ($packages): void {
    ...
    public function performSync()
    {
        ...
        if (config('app.debug')) {
            if (!is_dir(storage_path() . '/logs/dumps')) {
                mkdir(storage_path() . '/logs/dumps', 0o770, true);
            }
