...
class APIController extends Controller
{
    ...
    public function eos()
    {
        ...
        foreach ($goods as $good) {
            ...
            $item = [
                ...
                'allQnts' => [
                    'prepaid' => $good->availablePackages->filter(fn ($package) => $package->preorder)->map(function ($package) {
                    ...
                    'instant' => $good->availablePackages->filter(fn ($package) => !$package->preorder)->map(function ($package) use ($good) {
                        /** @var GoodsPackage $package */
                        ...
                        $zones = $positions->map(function ($position) {
                            ...
                            return null;
                        })->filter(fn ($i) => null !== $i)->map(fn ($title) => str_replace(' район', '', $title))->unique()->values();
        ...
        return response()->json([
            'response' => [
                ...
                'position' => [
                    'id' => $position->id,
                    'good_id' => $good->id,
                    'created_at' => (float) $good->created_at->format('U'),
    ...
    public function qiwi()
    {
        ...
        $wallets = QiwiWallet::whereStatus(QiwiWallet::STATUS_ACTIVE)->where(function ($query): void {
            $query->whereNull('last_checked_at')->orWhere(
                'last_checked_at',
                '<=',
        ...
        $sortedOrders = [];
        $orders->each(function ($order) use (&$sortedOrders): void {
                ...
                'packages' => $good->availablePackages->map(
                    function ($package) {
                        ...
                        $zones = $positions->map(function ($position) {
                            ...
                            return null;
                        })->filter(fn ($i) => null !== $i)->unique()->values();
