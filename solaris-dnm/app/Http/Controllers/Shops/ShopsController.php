...
class ShopsController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            if (null !== $this->propertiesProvider->getBool(DynamicPropertiesProvider::KEY_ENABLED)) {
    ...
    public function pageAdd(Request $request, $slug)
    {
        ...
        EmployeesLog::log(
            \Auth::user(),
            EmployeesLog::ACTION_SETTINGS_PAGE_ADD,
            ['page_id' => $page->id],
    ...
    public function pageEdit(Request $request, $slug, $pageId)
    {
        ...
        EmployeesLog::log(
            \Auth::user(),
            EmployeesLog::ACTION_SETTINGS_PAGE_EDIT,
            ['page_id' => $page->id],
            ['page_title' => $page->title]
        );
    ...
    public function pageDelete(Request $request, $slug, $pageId)
    {
        ...
        EmployeesLog::log(
            \Auth::user(),
            EmployeesLog::ACTION_SETTINGS_PAGE_DELETE,
            ['page_id' => $page->id],
            ['page_title' => $page->title]
        );
    ...
    public function good(Request $request, $slug, $goodId)
    {
        ...
        /** @var Builder|GoodsPackage[] $packages */
        $packages = $good->availablePackages();

        /** @var null|City $selectedCity */
        $selectedCity = null;
        ...
        if (null !== $cityId) {
            $packages = $packages->filter(fn ($package) => $package->city_id == $cityId);
        }
    ...
    public function showBuyForm(Request $request, $slug, $goodId, $packageId)
    {
        ...
        $isRiskyOperation = $package->getPrice(BitcoinUtils::CURRENCY_RUB) >= 100000;
        $isRiskAccepted = (bool) ($request->get('accepted', false));
    ...
    public function buy(ShopBuyRequest $request, $slug, $goodId, $packageId)
    {
        ...
        return redirect('/orders/' . $order->id, 303)->with(
            'flash_success',
