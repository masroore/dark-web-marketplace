...
class ServiceController extends Controller
{
    ...
    public function orders(Request $request)
    {
        ...
        return view('shop.service.orders', compact(
            'shop',
            'orders',
            'goods',
            'cities',
    ...
    public function financesToggle(FinancesToggleRequest $request)
    {
        ...
        return redirect('/shop/service/finances')
            ->with(
                $request->get('enabled') ? 'flash_success' : 'flash_warning',
    ...
    public function newThread(SecurityServiceAddThreadRequest $request)
    {
        ...
        switch ($request->get('receiver')) {
            case 'user':
                $employee = $shop->employees()->where(fn ($query) => $query->where('role', Employee::ROLE_OWNER)->orWhere('sections_messages_private', true))->findOrFail($request->get('receiver_id'));
