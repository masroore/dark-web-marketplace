...
class OrdersController extends ManagementController
{
    ...
    public function index(Request $request)
    {
        ...
        if (!Auth::user()->can('management-sections-orders')) {
            $orders = $orders->where(function ($query): void {
                $query->whereHas('position', function ($query): void { // quests made by employee
                    $query->where('employee_id', Auth::user()->employee->id);
                });

                if (Auth::user()->can('management-quests-preorder')) { // employee can make preorders for own
                    $query->orWhere(function ($query): void {
                        $accessibleGoodsIds = $this->shop->goods->filter(fn ($good) => Auth::user()->can('management-quests-create', $good))->pluck('id');
    ...
    public function order(OrderAddAddressRequest $request, $orderId)
    {
        ...
        EmployeesLog::log(
            Auth::user(),
            EmployeesLog::ACTION_ORDERS_PREORDER,
    ...
    public function orderNotFound(Request $request, $orderId)
    {
        ...
        if (!in_array($order->status, [Order::STATUS_PAID, Order::STATUS_PROBLEM])) {
