...
class StatsController extends ManagementController
{
    ...
    public function orders(Request $request)
    {
        ...
        [$periodStart, $periodEnd] = $this->parseDayPeriod(
            $request,
            $defaultStartDate,
            $defaultEndDate,
            $minStartDate
    ...
    {
        $amountDiff = (float) ($request->get('amount')) - $request->lot->amount;
    ...
    public function accountingDistributionAdd(ShopStatsAccountingDistributionAddRequest $request, $lotId)
    {
        ...
        $distribution->amount = ($distribution->amount ?: 0) + (float) ($request->get('amount'));
    ...
    {
        $amountDiff = (float) ($request->get('amount')) - $request->distribution->amount;
    ...
    public function filling()
    {
        ...
        foreach ($goods as $good) {
            ...
            $item = collect([
                'id' => $good->id,
                'title' => $good->title,
                'city' => $good->cities->map(fn ($city) => $city->title)->implode(', '),
                'packages_count' => $packages->count(),
                'packages' => $packages->map(fn ($package) => $package->getHumanWeight() . ' - ' . $package->getHumanPrice())->implode(chr(10)), // new line for tooltip
                'quests_count' => 0,
                'quests' => collect(),
                'quests_total' => collect([
                    BitcoinUtils::CURRENCY_BTC => 0,
            ...
            foreach ($packages as $package) {
                ...
                $item['quests'][] = sprintf(
                    ...
                    $questsCount,
                    plural($questsCount, ['квест', 'квеста', 'квестов'])
                );
            ...
            })->map(fn ($amount, $currency) => human_price($amount, $currency))->implode(', ');
