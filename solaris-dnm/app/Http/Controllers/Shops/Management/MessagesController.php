...
class MessagesController extends ManagementController
{
    ...
    public function thread(Request $request, $threadId)
    {
        ...
        $employees = [
            'participants' => $allEmployees->filter(fn ($employee) => in_array($employee->user->id, $participants) && $employee->id > 1),
            'rest' => $allEmployees->filter(fn ($employee) => !in_array($employee->user->id, $participants) && $employee->id > 1),
        ];
        unset($allEmployees);
