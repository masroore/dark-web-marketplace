...
class DiscountsController extends ManagementController
{
    ...
    public function promoAdd(Request $request)
    {
        ...
        Promocode::create([
            ...
            'percent_amount' => $request->get('discount_mode') == Promocode::DISCOUNT_MODE_PERCENT
                    ? $request->get('percent_amount')
                    : null,
            'price_amount' => $request->get('discount_mode') == Promocode::DISCOUNT_MODE_PRICE
                    ? $request->get('price_amount')
                    : null,
            'price_currency' => $request->get('discount_mode') == Promocode::DISCOUNT_MODE_PRICE
    ...
    public function showGroupsMasterForm(Request $request)
    {
        $users = User::with(['group'])
            ...
            ->get()
            ->filter(fn ($user) => null !== $user->suggestDiscountGroup());
